function N = is_vertex_nonmanifold(F)
  ...
  n = max(F(:));
  m = size(F,1);
  % This is supposedly memonnry inefficient, but the vectorization in matlab seems
  ...
  if vectorized 
    ...
    U = U & (UV*UV')>0;
    % U(i,j) = 1 meisans unique face-for-vertex i comes from same vertex AND shares
