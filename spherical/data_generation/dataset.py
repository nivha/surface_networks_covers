# Based on code from https://github.com/jonas-koehler/s2cnn
...
def render_model(mesh, sgrid, include_loc=False):
    ...
    if include_loc:
        loc_im = np.zeros((3, sgrid.shape[0]))
        loc_im[:, index_ray] = loc.T
        im = np.concatenate((im, loc_im), axis=0)
...
class ProjectOnSphere:
    def __init__(self, bandwidth, include_loc=False, loc_only=False, sphere_grid_path=None):
        self.bandwidth = bandwidth
        self.include_loc = include_loc
        self.loc_only = loc_only
        # self.sgrid = make_sgrid(bandwidth, alpha=0, beta=0, gamma=0)

        # load our grid instead
        # sphere_grid_path = 'ours_sphere_grid.mat'
        self.sgrid = loadmat(sphere_grid_path)['V']

    def __call__(self, mesh):
        im = render_model(mesh, self.sgrid, self.include_loc)
